...
async def list_strategies(
    ...
    response = supabase.table("tv_strategies").select("*").eq("user_id", current_user.id).execute()
    
    # Add webhook URLs
    strategies = []
    for s in response.data:
        s["webhook_url"] = f"{settings.BASE_URL}/api/v1/webhooks/tv/{s['webhook_secret']}"
        strategies.append(s)
...
async def update_strategy(
    ...
    supabase.table("tv_strategies").update(update_data).eq("id", strategy_id).execute()
...
async def delete_strategy(
    ...
    # Delete (cascade will handle signals)
    supabase.table("tv_strategies").delete().eq("id", strategy_id).execute()
