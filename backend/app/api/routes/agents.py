...
async def submit_job_result(
    ...
    job_response = supabase.table("jobs").select("job_type, input_data").eq("id", job_id).execute()
        ...
        if job_type == "compile":
            ...
            if version_id:
                supabase.table("ea_versions").update({
                    "status": "compiled" if request.status == "completed" else "failed",
            ...
            if signal_id:
                supabase.table("tv_signals").update({
                    "status": "executed" if request.status == "completed" else "failed",
                    "fill_price": request.output_data.get("fill_price") if request.output_data else None,
                    "broker_order_id": request.output_data.get("order_id") if request.output_data else None,
                    "error_message": request.error_message,
                    "resolved_at": "now()",
